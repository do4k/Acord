...
<script lang="ts">
	...
	let error = '';
	let messages: Array<string | { type: 'gif'; data: string; author: string }> = [];
	let messageToSend = '';
	...
	function handleGifUpload(event: Event) {
		...
		if (file && file.type === 'image/gif') {
			...
			reader.onload = () => {
				if (ws && connected) {
					ws.send(JSON.stringify({ type: 'gif', data: reader.result, author: username }));
				}
			};
		...
		<section style="position:fixed; top:0; left:0; width:100vw; height:100vh; background:#222; color:#f5f5f5; display:flex; flex-direction:row; align-items:stretch;">
			<div style="flex:4; display:flex; flex-direction:column; height:100vh;">
					...
					{#each messages as msg, i}    
						{#if typeof msg === 'object' && msg.type === 'gif'}
							<div style="align-self:flex-start; max-width:70%; text-align:left;">
								<div style="font-weight:bold; color:#1976d2; margin-bottom:2px;">{msg.author}</div>
								<img src={msg.data} alt="GIF" style="max-width:200px; border-radius:8px; margin-bottom:4px;" />
							</div>
				...
				<div style="width:100%; display:flex; gap:0.5em; padding:1em; background:#222; border-top:1px solid #333;">
					...
					<label for="gif-upload" style="margin-left:1em; cursor:pointer; background:#1976d2; color:#fff; padding:0.5em 1em; border-radius:4px; font-size:1em; display:flex; align-items:center; gap:0.5em;">
						<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" viewBox="0 0 20 20"><path d="M10 2a6 6 0 0 0-6 6v3.586l-.293.293A1 1 0 0 0 4 14h12a1 1 0 0 0 .707-1.707l-.293-.293V8a6 6 0 0 0-6-6zm-4 6a4 4 0 1 1 8 0v4H6V8zm-1 6v-1h10v1H5z"/></svg>
						Upload GIF
						<input id="gif-upload" type="file" accept="image/gif" on:change={handleGifUpload} style="display:none;" />
					</label>
				</div>
			</div>
